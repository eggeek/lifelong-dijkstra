...
inline bool operator!=(Arc l, Arc r){
  ...
  inline bool cmp(Arc& lft, Arc& rgh) {
    if (lft.source == rgh.source) {
      return lft.target < rgh.target;
    }
    else return lft.source < rgh.source;
  }
...
struct ListGraph{
  ...
  void sort() {
    std::sort(arc.begin(), arc.end(), cmp);
  }
};
...
ListGraph extract_node_induced_subgraph(
  ...
  for(auto a:g.arc)
    if(g_to_s[a.source] != -1 && g_to_s[a.target] != -1)
      s.arc.push_back({g_to_s[a.source], g_to_s[a.target], 0, 0});
